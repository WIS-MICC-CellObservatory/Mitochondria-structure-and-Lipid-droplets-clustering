...
#@ Integer(label="LD intensity % threshold",value="10", persist=true, description="the lowest % intensity LD to be ignored") iLDIntensityThreshold
#@ Float(label="LD Max size",value="3", persist=true, description="the lowest % intensity LD to be ignored") iLDMaxSize
...
#@ Boolean(label="Use Stardist segmentation for LD",value=true, persist=true, description="for quicker runs, use previous Ilastik results, if exists") iUseStardistSegmentation
...
function GetLDsRois()
{
	...
	gNumLDs = FilterRois(gLDZimageId, 0,-1,"Area","<", iLDMaxSize);
...
function GetLDsLabeledImageWrapper(imageId){
	if(iUseStardistSegmentation){
		return GetLDsLabeledImage1(imageId);
	}
	else{
		return GetLDsLabeledImage2(imageId);
	}
}
function GetLDsLabeledImage2(imageId)
{
	tmpROIFullPath = gImagesResultsSubFolder+"/"+gTempROIFile;
	...
	run("Divide...", "value=255.000");
		waitForUser("4");
	filtered8BitImage = getTitle();
	imageCalculator("Multiply create 16-bit", zProject,filtered8BitImage);
	waitForUser("5");
	filteredImageId = getImageID();
	//run("Watershed");
	//waitforuser("check watershed image");
	gLDsLabelsImageId = RunStarDistModel(filteredImageId);
	//setThreshold(1, 1000);
	//run("Analyze Particles...", "size=0.0001-infinity add show=Outlines display include");
	gNumLDs = roiManager("count");
